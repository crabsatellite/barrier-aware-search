...
\ecjHeader{x}{x}{xxx-xxx}{200X}{Covariance-Induced Irreversible Drift}{Alex Chengyu Li}

\title{\bf Cross-Dimensional Covariance Adaptation Induces Irreversible Drift
Toward Deceptive Attractors in Barrier-Constrained Search}

\author{%
\name{\bf Alex Chengyu Li} \hfill \addr{cl7076@nyu.edu}\\
...
search problem with a non-smooth objective and binary feasibility
constraints, full CMA-ES initially discovers feasible, low-overhead
solutions but progressively amplifies redundant parameter dimensions
through cross-dimensional covariance adaptation, trading constraint
feasibility for marginal objective improvement. In a 30-seed controlled
comparison, full CMA-ES drifts in 73\% of runs versus 43\% for
sep-CMA-ES and 16\% for isotropic ES (Fisher's exact $p = 0.018$,
Mann-Whitney $p = 0.001$). Extended 3000-generation escape attempts
from drifted endpoints reveal asymmetric irreversibility: full CMA-ES
...
learned covariance structure maintains the lock. The underlying
landscape contains feasible basins not connected by any tested linear
path; covariance adaptation induces directed, asymmetric transitions
across this basin structure. Controlled ablations
on the actual problem confirm that cross-dimensional interactions (C2)
are the primary necessary condition ($p = 0.0002$ when Spearman is
replaced with MSE). Population scaling experiments show widening drift
separation with increasing population size, from 30 percentage points
at $\lambda = 17$ to complete separation (100\% vs.\ 0\%) at
$\lambda = 200$, consistent with improved covariance estimation
as the causal mechanism. Adaptive mitigation via reactive
switching from full to sep-CMA-ES upon drift detection fails (70\%
drift rate), indicating that preemptive variant selection is necessary.
\end{abstract}
...
\begin{keywords}
  ...
  basin topology, directed transition graph
\end{keywords}
...
\citet{hansen2001completely,hansen2016cma}) is among the most effective
...
\item \textbf{Directed basin topology.} We show that the
  barrier-constrained fitness landscape contains at least three
  empirically disconnected feasible basins harboring qualitatively
  distinct solution types, linked by directed transitions under
  covariance adaptation but not in reverse
  (Sections~\ref{sec:landscape} and~\ref{sec:mechanism}).
...
In our setting, the deception is \emph{optimizer-specific}:
...
\textbf{Fitness landscape topology.}
The topological structure of fitness landscapes (connected components,
barrier heights, basin volumes) has been studied in both
evolutionary computation~\citep{reidys2002combinatorial} and computational
chemistry~\citep{wales2003energy,becker1997topology}. In energy landscapes,
\emph{disconnectivity graphs} characterize the basin structure through
hierarchical trees of local minima connected by transition states. Our
finding that the feasible set decomposes into empirically disconnected
components with directed inter-basin transitions parallels this framework
in constrained optimization, where the transition structure depends on the
optimizer's adaptation mechanism as much as the landscape geometry.
...
\item \textbf{Algebrization}~\citep{aaronson2008algebrization}: the fraction of
  ...
  autocorrelation sum, all defined via polynomial or Fourier structure
...
region. The intersection of all three creates the empirically disconnected basin landscape
...
implementations. The qualitative landscape structure (empirically disconnected feasible basins)
...
%% ============================================================
\section{Topology of the Compliance Landscape}
\label{sec:landscape}
%% ============================================================
...
\subsection{Path-Disconnection of Feasible Basins}
...
(6 representative paths of 10; full table in Appendix~\ref{app:interpolation}).
...
Subject to this caveat, in the language of landscape
topology~\citep{reidys2002combinatorial}, the feasible set $\Theta_{\text{feas}} = \{\btheta \in \R^{85} :
\text{NP}(\btheta) \wedge \text{Rel}(\btheta) \wedge
\text{Alg}(\btheta)\}$ contains at least three components that are
not connected by any tested linear path, each harboring a distinct
solution type. The depth of the separating valleys ($\rho$ dropping
to $-0.16$, NP density reaching 1.000) is consistent with genuine topological disconnection, though we
have not excluded the existence of nonlinear feasible paths in the
85-dimensional parameter space. As we show in
Section~\ref{sec:mechanism}, these disconnected basins are linked by
directed, optimizer-dependent transitions, forming a structure we
describe as a directed transition graph.
...
the within-seed trajectory data confirms this: seed~42's
...
frozen covariance (4/5) confirms that the learned covariance structure,
rather than the landscape itself, maintains the lock.}
...
The basin structure (Section~\ref{sec:landscape}) and the
asymmetric drift motivate viewing the basins as nodes in a \emph{directed
transition graph}: full CMA-ES induces directed edges $\text{C} \to \text{B}$ (frequent)
and $\text{B} \to \text{A}$ (observed in 1/5 long runs) but not the
reverse direction (escape probability $\leq 1/10$). This directed structure is
optimizer-dependent: sep-CMA-ES does not traverse the same edges, and
the graph collapses to isolated nodes under isotropic search. The
transition structure is thus a joint property of the optimizer and
the landscape.
...
$\lambda = 50$ is used in all other experiments. The widening
...
\begin{tabular}{lcccc}
\toprule
$\lambda$ & Full drift & Sep drift & Gap (pp) & Fisher $p$ (two-sided) \\
\midrule
...
learning of sensitivity--signal cross-dimensional couplings. This widening gap
directly supports the covariance-learning mechanism: if drift were caused by some other property of full CMA-ES
...
unconstrained objective but cross constraint boundaries. Unlike the step-size difficulties reported for smooth constraints, where the
...
\subsection{Directed Transition Graphs in Constrained Landscapes}

The basin decomposition and directed drift reported here are
reminiscent of the \emph{disconnectivity graph} framework used in energy
landscape theory~\citep{wales2003energy,becker1997topology} to
characterize the topology of potential energy surfaces. In that
framework, local minima are connected by saddle points, and the
resulting graph captures the kinetic accessibility of different basins.
In our setting, however, the transition structure depends on both
the landscape and the optimizer's adaptation mechanism. Full CMA-ES traverses directed edges between
basins that sep-CMA-ES does not.

This optimizer-dependent structure suggests a conceptual framework:
for a given adaptive optimizer on a constrained landscape, one can
define a \emph{directed transition graph} whose nodes are the
empirically identified basin components and whose edges record the
observed transition frequencies between them. The graph's
structure, particularly the existence of near-trapping nodes (basins
from which the optimizer escapes with low probability), shapes the
optimizer's long-run behavior. Our results show that full CMA-ES's
transition graph contains near-trapping nodes (Type~A/B, escape
probability $\leq 1/10$) that sep-CMA-ES's graph does not, providing
a structural explanation for the variant-dependent drift rates.

Whether this framework generalizes beyond CMA-ES to other adaptive
optimizers on constrained landscapes is an open question. The structural
conditions identified in Section~\ref{sec:irreducibility} (non-smooth
objectives, cross-dimensional interactions, discontinuous
constraints) may indicate settings where analogous directed transition
structures arise in other optimizer families.
...
experiments show widening drift separation, reaching complete
separation (100\% full vs.\ 0\% sep) at $\lambda = 200$, consistent
with improved covariance estimation as the causal mechanism.
...
The disconnected basins and directed transitions observed here
suggest that constrained optimization landscapes can contain
empirically disconnected feasible components linked by
optimizer-dependent transitions that shape long-run behavior.

The open questions are external validity (does the phenomenon
arise on other constrained optimization problems with C1--C3
structure?) and whether the directed transition graph framework provides
a general diagnostic for optimizer--landscape interactions beyond the
...
\url{https://github.com/crabsatellite/barrier-aware-search}.
\appendix
...
17 & Covariance dissection & Lock is covariance-reinforced \\
...
reveals that 3/13 are critical for feasibility (run\_length$\times$sensitivity,
